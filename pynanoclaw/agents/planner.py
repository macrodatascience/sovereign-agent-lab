...
- If searching, step must call search_web
- If evaluating candidates, explicitly list each candidate evaluation separately
- If information must be structured, assume a parser function will handle extraction
...
def plan_task(task: str) -> list[str]:
    ...
    if "edinburgh" in task_lower and "venue" in task_lower:
        return [
            "check_pub_availability for The Albanach",
            "check_pub_availability for The Haymarket Vaults",
            "check_pub_availability for The Guilford Arms",
            "check_pub_availability for The Bow Bar",
        ]
